...
uploadImageOption.defalutFormat = [[], [56, 56], [128, 128], [640, 640]]
/**
 * 格式化图片地址
 * @param input
 * @param args
...
uploadImageOption.formatImage = function (input, args) {
  ...
  if (input.substring(0, 4) === 'http' || input.substring(0, 10) === 'data:image' || input.substring(0, 7) === 'imageid' || input.substring(0, 9) === 'blob:http') {
    return input
  }
  if (args) {
    // 是否为编辑状态：
    ...
    if (isEdit) {
      let width = args[3] || 0
      let height = args[4] || 0
      return 'http://' + input + '_' + width + 'x' + height + '_-1x-1_' + width + 'x' + height
    } else {
      let width = args[0] || 0
      let height = args[1] || 0
      return 'http://' + input + '_' + width + 'x' + height + '_-1x-1_' + width + 'x' + height
