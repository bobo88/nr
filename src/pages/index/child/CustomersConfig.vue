<template>
  <div class="customers-config">
    <div class="mb10">
      <el-button type="success" size="small" @click="addOprate">添加合作客户图</el-button>
    </div>

    <el-table
      :data="bannerConfigData"
      border
      style="width: 100%">
      <el-table-column
        label="Customers"
    ...
    <el-dialog
      ...
      <div class="tc-main">
        <div class="mb10">
          <span class="inline-block w100">Customers：</span>
          <div class="w300 inline-block">
            ...
            <el-upload
              ...
              :action="actionCustomers()"
              :on-success="handleAvatarSuccess"
              multiple>
              <el-button class="vm" size="small" type="primary">点击上传Customers</el-button>
            </el-upload>
          </div>
        </div>
        <div class="mb10">
    ...
    methods: {
      getCustomers () {
        ...
        .then(data => {
          if (data.err === 0) {
            let _data = data.data;
            this.bannerConfigData = _data.map((item) => {
              item.logo = this.Config.IMG_BASE_URL + item.logo;
              return item;
            });
            console.log('Customers data->', _data);
          } else {
